...
public class PipelineHook implements MarkDownMsg {
    ...
    public String getMarkdown() {
        ...
        if (!"running".equals(status)) {
            ...
            sb.append(String.format("[\uD83D\uDEAB取消运行](%s%s?projectId=%s&pipelineId=%s&action=cancel) ", hostSchema, pipelineCancelDeleteUrl, project.getId(), objectAttributes.getId()));
            sb.append(String.format("[♻️重新运行](%s%s?projectId=%s&pipelineId=%s&action=retry) ", hostSchema, pipelineCancelDeleteUrl, project.getId(), objectAttributes.getId()));
            sb.append(String.format("[⛔删除](%s%s?projectId=%s&pipelineId=%s&action=delete) %n%n", hostSchema, pipelineCancelDeleteUrl, project.getId(), objectAttributes.getId()));
        }
        return sb.toString();
    }
